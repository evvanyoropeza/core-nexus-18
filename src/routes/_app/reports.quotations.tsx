...
import { getQuotationsReport } from "@/lib/reports.functions";
...
export const Route = createFileRoute("/_app/reports/quotations")({
  component: QuotationsReport,
});
...
function QuotationsReport() {
  ...
  const fetchReport = useServerFn(getQuotationsReport);

  const { data, isLoading } = useQuery({
    queryKey: ["report", "quotations", currentTenant?.tenant_id, period, status],
  ...
  return (
    <ReportShell
      ...
      subtitle={`Periodo: ${period.from} → ${period.to} · ${rows.length} cotizaciones`}
      onExport={() =>
        downloadCSV(
          `cotizaciones_${period.from}_${period.to}.csv`,
          ["Folio", "Fecha", "Cliente", "Estatus", "Subtotal", "Impuestos", "Total"],
          rows.map((r) => [r.folio, r.issue_date, r.customer_name, STATUS_LABEL[r.status as keyof typeof STATUS_LABEL] ?? r.status, r.subtotal.toFixed(2), r.tax_amount.toFixed(2), r.total.toFixed(2)]),
        )
      }
      filters={
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell><Badge variant="outline">{STATUS_LABEL[r.status as keyof typeof STATUS_LABEL] ?? r.status}</Badge></TableCell>
            ...
            {rows.length === 0 && (
              <TableRow><TableCell colSpan={7} className="text-center text-sm text-muted-foreground py-6">Sin cotizaciones</TableCell></TableRow>
            )}
          </TableBody>
          {rows.length > 0 && (
